/* This example configures the Teensy as a MIL-1553 Remote Termianl.
   This will wait for the BC to send a data packet, it will acknowledge
   the packet, and will display the packet contents on the serial port
...
void setup() {
   ...
   if(!myRemoteTemrinal.openMailbox(MB1_SA, MB1_WC, MIL_1553_RT_INCOMING, &mailBox1))
      Serial.println("Open mailbox1 failed");

   //Setup mailbox 2
   // assign mailbox to specified subaddress
   if(!myRemoteTemrinal.openMailbox(MB2_SA, MB2_WC, MIL_1553_RT_INCOMING, &mailBox2))
...
void loop()
{
   ...
   // first one that has new data. A mailbox and a packet are really the
   // same thing. PACKET refers to the data structure, and a MAILBOX is
   // more of an analogy.
   MIL_1553_packet* packet = myRemoteTemrinal.mailAvailable();
   if(packet != NULL) {
      printPacket(packet);
      // depending on which getData() method is called to read the packet
      // data, you might need to manually clear the newMail flag, so that
      // the mail box knows that you have picked up the mail.
